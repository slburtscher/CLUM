...
def peakdetect(y_axis, x_axis = None, lookahead = 300, delta=0):
    ...
    max_peaks = []
    min_peaks = []
    peaks =[] # slb min und max peaks in einer List bzw File
    ...
    for index, (x, y) in enumerate(zip(x_axis[:-lookahead], 
        ...
        if y < mx-delta and mx != np.Inf:
            ...
            if y_axis[index:index+lookahead].max() < mx:
                ...
                peaks.append([mxpos, mx]) # slb min und max peaks in einer List bzw File
                dump.append(True)
        ...
        if y > mn+delta and mn != -np.Inf:
            ...
            if y_axis[index:index+lookahead].min() > mn:
                ...
                peaks.append([mnpos, mn]) # slb min und max peaks in einer List bzw File
                dump.append(False)
    ...
    try:
        if dump[0]:
            max_peaks.pop(0)
            peaks.pop(0) # slb min und max peaks in einer List bzw File
        else:
            min_peaks.pop(0)
            peaks.pop(0) # slb min und max peaks in einer List bzw File
        del dump
